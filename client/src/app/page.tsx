...
function TrustStrip({ totalCount }: { totalCount: number }) {
  ...
  const items = [
    ...
    { key: 'protection', label: 'Buyer protection' },
    { key: 'australia', label: 'Australia-wide' },
    { key: 'trending', label: countLabel },
  ];

  return (
    <div className="mt-12 grid grid-cols-2 gap-y-4 gap-x-6 border-t border-[var(--border-subtle)] pt-6 sm:grid-cols-4">
      {items.map((item, i) => (
...
function TrustIcon({ name }: { name: string }) {
  ...
  switch (name) {
    ...
    // Padlock — clearly a different shape from the verified seal
    case 'protection':
      return (
        <svg className={common} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.75">
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75M6.75 10.5h10.5a1.5 1.5 0 011.5 1.5v7.5a1.5 1.5 0 01-1.5 1.5H6.75a1.5 1.5 0 01-1.5-1.5V12a1.5 1.5 0 011.5-1.5z"
          />
        </svg>
      );
